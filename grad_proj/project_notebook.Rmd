...
md_2020_county_election_results <- read_csv("data/md_2020_county_election_results.csv") %>%
  clean_names() %>%
  #chatGPT helped me with the next few lines
  rowwise() %>%
...
These counties have drastically different populations, so I needed to incorporate ACS data to calculate per capita migration levels which would allow me to compare levels across counties.
...
There's heteroscedasticity. My data also isn't normally distributed -- let's visualize that with a histogram to check the extent of the problem:
