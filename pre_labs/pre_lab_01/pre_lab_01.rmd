...
In the space below, write the name of the department from the first row of the "title" column.
...
specify the data frame we want to do stuff with: umd_courses |>
Group / consolidate the data, categorized by the term: group_by(term) |>
Performs the aggregations on grouped data and creates a new df: summarise
Creates a column in the output df where the values are the number of records in each term: count_classes = n()
...
Arranges the df by the values in count_classes in descending order
...
It's grouped by both term AND department, thus adding another column and breaking the groups out further
...
Groups only by term, but sums up the total number of seats available per term and
adds that as a new column total_seats
...
calculates the mean and median number of seats per group and adds those calculations as new cols.
The top six terms are all Fall terms
...
Groups the df by department instead of term.
Calculates and adds cols for min and max # of seats
Data is sorted by total seats in descending order
...
1.  Is there a general trend of some departments losing classes over time while others gain classes?
    Could visualize count_classes by department with line graphs to get a quick view of this

2.  What classes have been consistently offered from 2017 (the earliest term in the data set) through today?

3.  Which instructor teaches the most classes per term on average?
