...
It's important because if we don't store the results in any variable then we would have to keep re-typing the code any time we wanted
to use this smaller data set in the future. (select is a non-mutating function.) But we want to store it in a new variable rather that reassigning the output to journalism_courses because otherwise we'd lose the ability to work with the unmodified version of journalism_courses
...
Using the OR operator significantly expands the criteria for permitted records, returning 41011 records instead.
...
It has changed the object type to Date instead of character, which also reformats the order to year-month-date
...
All days are being set to 01 as a method of grouping dates within the same month (and year) together
...
1) adds a new column "total_votes" to the df, where each row is equal to the sum of values in cox + moore + lashar + wallace + write_ins in the same row
2) adds a new column "pct_moore" to the df, where each row is equal to the value of moore divided by total_votes in the same row
...
The values are now whole numbers representing percentages, rather than being presented as decimals.
...
It depends on what question(s) you're trying to ask. If you're interested in which counties voted the LEAST for Moore then sure, otherwise it might not be.
...
1) str_to_lower() converts to lower case

2) str_to_sentence() capitalizes the first letter of a sentence
...
- creates a new column named state
- if "Maryland" is anywhere in the address value for that row, populate that row's "state" value with "MD"
- if not, then if "California" is anywhere in the address value for that row, populate that row's "state" value with "CA"
- etc for the rest of the states in that code
- if none of those states are present in the address line, make the value NA
...
Yes. I would have expected the most common records for this particular data set to be related to MD addresses since it's a data set of Maryland political committee expenses. NA makes sense as # 2 because all states other than those listed in our str_detect code is being set as NA
...
1. Remember how we had some courses in the UMD course data where Instructor was a list of people rather than just one person? If we wanted to ask how many courses any particular professor has taught, we could incorporate str_detect to ensure we capture courses where that instructor is included in a longer list of instructors

2. What's the total expenditure amount for expenses purposes related to "Wages"? (str_detect again)

3. Filter the expenditure data to individual expenditure amounts greater than $100 (filter)
