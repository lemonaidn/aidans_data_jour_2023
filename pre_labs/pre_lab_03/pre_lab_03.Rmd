...
Calling problems() gives you additional information on the parsing error: 5 variables: row <int>, col <int>, expected <chr>, actual <chr>, file <chr>
...
Vendor Name is chr. Amount is dbl. Date is chr
...
Thousand of zip codes are less than five characters. Because R is interpreting Zip Codes as a dbl rather than chr, zip codes with leading 0's might be getting dropped. Foreign postal codes are included. Probably other unknown issues too
...
Some records are missing categories (showing as NA). 2009 is missing a Loan category. 2013 has a mystery "L" category which might be shorthand for Loan, but that's just an assumption. A single record in 2017 is listed as "contract." This is problematic because it affects the accuracy and certainty of our summary statistics/counts
...
There's a grant that was only $60? That seems pretty unusual
...
spaces in col names are converted to underscores and changed to lowercase
...
- stores the results of our work in cleaned_md_grants_loans, updating the original version of the df
- spaces in col names are converted to underscores and changed to lowercase
- renames the column "grantor" to "source"
- changes the values in the cols source, grantee, and description to all caps
- displays the updated df
...
There's a new zip5 column that's a maximum of 5 characters long
...
21202 has received the most, which makes sense because it's in downtown Baltimore. I'm not surprised that recipients from the epicenter of MD's most populous city would have received the highest total amount of money.
...
cleaned_md_grants_loans %>%
  ...
  group_by(zip5) %>%
  summarise(total_awards = sum(amount)) %>%
