...
```{r echo=FALSE}
census_api_key("d0dad2595c2515a7ebbaa971b4679f269f0812c3", install=TRUE)
```
...
"Median household income and margins of error by county"
...
No! They're within each other's the margin of error.

Is the difference between Howard and everyone else significant? **Answer**

Yes! It's outside the margin of error for all the other counties
...
Baltimore City has the largest increase. No county saw a decrease
...
Is the percent of students enrolled in college correlated with household income? What might make the answer harder to interpret is that some schools likely draw their student populations from outside the zip code they're located in -- meaning the demographics might not accurately reflect the student populatio for those schools
...
the first set of high schools have an avg income of just under 111k, while the second set has just under 70k
...
Is there any correlation between the size of the graduating class and college enrollment rates?

```{r}
library(ggplot2)
ggplot(md_high_schools_with_acs, aes(x=high_school_graduates, y=percent_of_high_school_graduates_enrolled)) + geom_point()
```

```{r}
cor.test(md_high_schools_with_acs$high_school_graduates, md_high_schools_with_acs$percent_of_high_school_graduates_enrolled)
```
