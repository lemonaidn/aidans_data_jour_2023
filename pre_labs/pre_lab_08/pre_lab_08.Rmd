...
- Create a new data frame called baltco_911_calls_by_month
- Update the month column to by date type
- groups by month
- calculates the total number of calls per month
- arranges the data from highest to lowest calls per month
...
A (colourless) bar chart! Displays the call count by month
...
It reorders the bar chart by count call rather than by month
...
The minimal theme changes the backdrop / grid behind the bars
...
The economist theme changed the backdrop / grid again
...
The title and axes' labels have changed and a caption was added in the bottom right of the graph
...
We now have a df with total calls by date rather than by month
...
We've created a line chart now!
...
It makes the visualization more granular on the x axis by making the grid weekly
...
We've rotated the dates on the x axis to make them slightly more legible
