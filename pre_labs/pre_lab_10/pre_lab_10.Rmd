...
OBJECTID, ZIPCODE2 (which is basically a unique geoid for the zip; 02400 represents Maryland and the rest of the digits are the 5-digit zip code), ZIPNAME, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area, geometry
...
It contains a df with the information described in our previous code chunk where we used glimpse() on the df
...
a map of Maryland with lat and long axes and boundaries drawn for each zip code
...
output: "Warning: package ‘tigris’ was built under R version 4.3.2To enable caching of data, set `options(tigris_use_cache = TRUE)`
...
A dataframe with every county and county-equivalent in the US along with state FIPS codes, geoid, land and water areas, and more
...
We've limited the data to Maryland dounties
...
A map of MD with lat and long axes and county boundaries
...
We've layered the zip and county boundaries on top of each other. The zip  code boundaries only include the land, whereas county boundaries include water too
...
a dataframe with only the zip codes and foreclosure numbers. There's no geometry included. Also, the first zip code is in Delaware?
...
we've joined the foreclosure df with the other df we made earlier containing a geometry col for each zip
...
identifies the center point o each county and displays the number of foreclosure notices in March using fill and scales the numbers with colors (darker colors = fewer foreclosures)
...
we've loaded MD foreclosure data by county and MD population data from the ACS by county. Note: this foreclosure data is structured in the csv so the counties are columns rather than rows, so we had to transpose the rows and cols
...
By adding geometry = TRUE to our code we've gotten a geometry col with the ACS data
...
It displays md_county_notices, which is a df with the number of notices in each county on each date (if we commented that line out it would display md_county_population instead, which is a df with population data for each county from the 2021 ACS)
...
", Maryland" has been removed from the county col, title case has been applied, and extra white space has been trimmed
...
joined the dataframes by county
...
we've calculated a per capita foreclosure col named "rate." Charles county has the highest rate by far
...
We've found the centroids of each county and created a map of MD that visualizes the per capita foreclosure rate by county with a color scale. County names have also been labeled, unlike our prior maps. Charles county stands out as having the highest foreclosure rate but the color scale doesn't make the extent of the difference super clear
...
We've adjusted the color scale to make the differences easier to see and interpret. (No more county labels though). CHarles County REALLY stands out now, which is appropriate based on its incomparable foreclosure rate
...
The colors of some of the counties changed. This is a positive change because the prior scale included levels that no county fell within, making it a less-effective visualization
