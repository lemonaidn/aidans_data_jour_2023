...
It's a dataframe with the date, title, url, and text of Ben Cardin's press releases
...
it shows you each of the unique values/strings within the vector (note: case matters)
...
It tokenizes the text and provides a unique list of words/tokens from the text (and because it lowercased the words and removed punctuation first, we don't have duplicates of the same words/tokens in different cases or with different punctuation)
...
Nope, because we mostly have a bunch of stop words.
...
So much better. Now we're starting to get an idea of the actual topics that were discussed the most
...
COVID is no longer in the top ten which reflects how it's not front-of-mind as much as it used to be. "Women" entered the top-ten in 2023, which I'm sure is connected to the Supreme Court overturning Roe the year before and Democrats realizing how potent of a political weapon abortion rights had become for them
...
It's a list of word pairs aka bigrams ordered by the ones that appear most often in the text
...
A few of them still aren't very informative -- project location, amount included... "location Baltimore" is a weird one to be said so many times. But the other results help give you an idea of some of the topics that Cardin speaks to / care most about. It's interesting that COVID-19 was still so high up for him in 2022.
...
for the most part, a ver different list! something else that's interesting -- the percent values for the very top results are much lower than the values from 2022
...
critical could really go either way -- the negative designation makes sense in some contexts, but there are also contexts where it could be used to mean "important" rather than something negative
